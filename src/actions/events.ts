...
export async function getEvents(): Promise<ActionResponse<EventData[]>> {
  try {
    ...
    return { success: false, error: "Erreur lors du chargement des événements" }
  }
}

export async function getEventById(id: string): Promise<ActionResponse<EventData | null>> {
  try {
    ...
    return { success: false, error: "Erreur lors du chargement de l'événement" }
  }
}

export async function getEventBySlug(slug: string): Promise<ActionResponse<EventData | null>> {
  try {
...
export async function createEvent(formData: FormData): Promise<ActionResponse<EventData>> {
  try {
    ...
    const { title, shortDescription, fullDescription, flyerImageUrl, eventDate, location } = validated.data
...
export async function updateEvent(id: string, formData: FormData): Promise<ActionResponse<EventData>> {
  try {
    const adminId = await getAdminUserId()
    if (!adminId) return { success: false, error: "Non autorisé" }
    ...
    if (updateData.title) {
      ...
      updateData.slug = slugExists ? `${newSlug}-${Math.random().toString(36).substring(2, 6)}` : newSlug
      updateData.publicUrl = generatePublicUrl(updateData.slug as string)
      updateData.qrCodeUrl = await QRCode.toDataURL(updateData.publicUrl as string, {
        width: 400,
        margin: 2,
        color: { dark: "#004749", light: "#ffffff" },
      })
...
export async function incrementScanCount(slug: string): Promise<ActionResponse<void>> {
  try {
...
export async function incrementDownloadCount(slug: string): Promise<ActionResponse<void>> {
  try {
    await prisma.event.update({
      where: { slug },
